...
public abstract class PageView extends ViewGroup {
	...
	public PageView(Context c, Point parentSize, Bitmap sharedHqBm, int mode) {
		super(c);
		mContext    = c;
	...
	public void setPage(int page, PointF size) {
		...
		mDrawEntire = new CancellableAsyncTask<Void, Void>(getDrawPageTask(mEntireBm, mSize.x, mSize.y, 0, 0, mSize.x, mSize.y)) {
			...
			public void onPostExecute(Void result) {
				...
				mBusyIndicator = null;
				if (viewMode == 1)
					mEntire.setImageBitmap(createInvertedBitmap(mEntireBm));
				else 
			...
			mDrawPatch = new CancellableAsyncTask<Void,Void>(task) {
				...
				public void onPostExecute(Void result) {
					...
					if (viewMode == 0)
						...
						mPatch.setImageBitmap(createInvertedBitmap(mPatchBm));
					mPatch.invalidate();
					//requestLayout();
