...
public class MuPDFReflowView extends WebView implements MuPDFView {
	...
	public MuPDFReflowView(Context c, MuPDFCore core, Point parentSize, int mode) {
		super(c);
	...
	public void setPage(int page, PointF size) {
		...
		mLoadHTML = new AsyncTask<Void,Void,byte[]>() {
			...
			protected byte[] doInBackground(Void... params) {
				return mCore.html(mPage, viewMode);
			}
			@Override
			protected void onPostExecute(byte[] result) {
				String test = new String(result, 0, result.length);
